...
torque_path = '/Volumes/GoogleDrive/Shared drives/Quantum Materials/MagnetTime/2022_08_TLH/week1/FeSb2-VT54/torque/'
...
axin = ax1.inset_axes([0.1, 0.12, 0.3, .4])
...
for i, s_name in enumerate(sweeps):
    ...
    field_2_check = field[locs]

    max_locs.append(field_2_check[np.argmax(V_2_check*1e5)])
    ...
    slope = np.polyfit(field[fit_locs], V[fit_locs]*1e5, 1)[0]

    axin.scatter(angles[i], slope, c=colours[i], s=275)

    ax1.plot(field, np.abs(V/1e-5), linewidth=2, c=colours[i])
    field, V = load_torque(resistance_path, s_name[1], R=True)
    ax1.plot(field, np.abs(V / 1e-5), linewidth=2, c=colours[i])



publication_plot(ax1, r'$\mu_0H$ (T)', r'$R$ ($\Omega$)')


publication_plot(axin, r'$\theta$ ($\degree$)', r'$R^\prime$ ($\Omega$T$^{-1}$)', label_fontsize=18, tick_fontsize=16)

max_R = np.array([angles, max_locs]).T

np.savetxt('/Volumes/GoogleDrive/My Drive/Figures/TLH_08_2022/VT54/max_R_locs.csv', max_R, delimiter=',')
...
plt.savefig('/Volumes/GoogleDrive/My Drive/Figures/TLH_08_2022/VT54/raw_data-VT54-resistance.pdf', dpi=300, bbox_inches='tight')
