...
for i, s_name in enumerate(sweeps):
    ...
    ax1.plot(field**2, tau*1e5, linewidth=2, c=colours[i], label=str(angles[i])+r'$\degree$')

    slope = np.polyfit(field[100:round(len(field)/4)]**2, tau[100:round(len(field)/4)]*1e5, 1)[0]
    grads.append(slope)
