...
sweeps = ['0.35K_37deg_sweep035_down.csv', # I = 100 uA
        '0.35K_37deg_sweep036_up.csv', #I = 100 uA
    '0.35K_37deg_sweep037_down.csv', # I = 1 mA
          '0.35K_37deg_sweep038_up.csv', # I = 1 mA
          '0.35K_37deg_sweep040_up.csv', # I = 2.5 mA
          '0.35K_37deg_sweep041_up.csv' # I = 2.5 mA
          ]

currents = [100e-6, 100e-6, 1e-3, 1e-3, 2.5e-3, 2.5e-3]

labels = ['0.1 mA', '0.1 mA', '1 mA', '1 mA', '2.5 mA', '2.5 mA']
...
conductance_quantum = .5 * (7.748091729 * 10 ** -5)
...
for i, s_name in enumerate(sweeps):
    ...
    interp_B = np.linspace(0.15, 41.1, 10000)
    interp_R = np.interp(interp_B, B, R)

    deriv_R = savgol_filter(interp_R,111,3,1)


    ax1.plot(B, R, linewidth=2, c=colours[i], label=labels[i])
    G = 1 / R / conductance_quantum

    if i > 3:
        Gs_2p5.append(1 / interp_R / conductance_quantum)
        Bs_2p5.append(interp_B)
    ax2.plot(B, G, linewidth=2, c=colours[i])

publication_plot(ax1, r'$B$ (T)', r'$R$ ($\Omega$)')
publication_plot(ax2, r'$B$ (T)', r'$G$ ($\frac{e^2}{h}$)')



axin = ax1.inset_axes([0.22,0.6,.45,.25])

diff_G = np.abs(Gs_2p5[0]  - Gs_2p5[1])

axin.plot(Bs_2p5[0], diff_G, linewidth=2, c='#EE9B00')

publication_plot(axin, r'$B$ (T)', r'$\Delta G$ ($\frac{e^2}{h}$)')
...
ax2.annotate(r'$\theta \approx 37 \degree$', xy=(0.22, 0.2), xycoords='axes fraction',
             ha="center", va="center", fontname='arial', fontsize=24)
ax2.annotate(r'$T = 1.5$ K', xy=(0.22, 0.1), xycoords='axes fraction',
             ha="center", va="center", fontname='arial', fontsize=24)
...
# plt.show()
plt.savefig('/Volumes/GoogleDrive/My Drive/Figures/TLH_08_2022/FSVT8-37degrees.pdf', dpi=300, bbox_inches='tight')
