...
field_root = 'Field_'
VT154_root = 'VT154_Cap_'
VT143_root = 'VT143_Cap_'
...
for i, s_num in enumerate(filenumbers):
    ...
    _, VT143 = load_xy(resistance_path + file_root + str(s_num) + file_end, str(s_num), x_root=field_root,
                     y_root=VT143_root, skiprows=9)



    ax1.plot(B, VT154, linewidth=2, c=colours[i], label=str(angles[i])+r'$\degree$')
    ax2.plot(B, VT143, linewidth=2, c=colours[i], label=str(angles[i])+r'$\degree$')

publication_plot(ax1, r'$B$ (T)', r'VT154 Torque (pF)')
publication_plot(ax2, r'$B$ (T)', r'VT143 Torque (pF)')
