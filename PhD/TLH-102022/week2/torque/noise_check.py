...
for i, s_num in enumerate(filenumbers):
    ...
    B, VT143 = load_xy(resistance_path+file_root+str(s_num)+file_end, str(s_num),x_root=field_root,
                     y_root=VT143_root,skiprows=9)
    ...
    f = np.poly1d(np.polyfit(B[locs], VT143[locs], deg=2))

    ax1.plot(B, VT143, linewidth=2, c='k', label='Raw')
    ax1.plot(B, f(B), linewidth=2, c='r', label='Fit')

    ax2.plot(B, VT143 - f(B), linewidth=2, c='gray')

publication_plot(ax1, r'$B$ (T)', r'Torque (V)')
publication_plot(ax2, r'$B$ (T)', r'Torque Residuals (V)')
...
ax1.set_xbound(0,42)
ax2.set_xbound(30,42)

ax2.set_ybound(-25e-9, 25e-9)
