...
for i, s_num in enumerate(filenumbers):
    ...
    _, VT16 = load_xy(resistance_path + file_root + str(s_num) + file_end, str(s_num), x_root=field_root,
                     y_root=VT16_root, skiprows=9)



    ax1.plot(B ** 2, VT15, linewidth=2, c=colours[i], label=str(angles[i])+r'$\degree$')
    ax2.plot(B ** 2, VT16, linewidth=2, c=colours[i], label=str(angles[i])+r'$\degree$')

publication_plot(ax1, r'$B$ (T)', r'VT15 Torque (pF)')
publication_plot(ax2, r'$B$ (T)', r'VT16 Torque (pF)')
