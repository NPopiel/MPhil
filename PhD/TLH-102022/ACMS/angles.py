...
for i, angle in enumerate(angles):
    ...
    deriv_X_up = savgol_filter(savgol_filter(medfilt(X_up,3), 5*oneT_window, polyorder,deriv=1), 5*oneT_window, polyorder)
    deriv_X_dn = savgol_filter(savgol_filter(medfilt(X_dn, 3), 5*oneT_window, polyorder, deriv=1), 3*oneT_window, polyorder)
    ...
    ax2.plot(field_dn, deriv_X_dn * 1e8, linewidth=2, c=colours[i])
    ...
    ax3.plot(field_dn, subtracted_deriv*1e8 - 1.1*i, linewidth=2,c=colours[i])

    # fig2, ax = MakePlot().create()
    #
    # ax.plot(field_dn, subtracted_deriv*1e8 - 1.1*i, linewidth=2,c=colours[i])
    # plt.show()
...
publication_plot(ax3, r'$\mu_0 H$ (T)', r'$\Delta\frac{d\chi^\prime}{dB}$ (arb.)')#^\prime
...
r1r2_ac_fields = np.array([8.6, 11.6, 11.9, 10.5, 13.5, 13.4, 12.4, 13])

r1r2_gilles_fields = np.array([6, 11.5, 13.5, 13.2, 14.5 ])
r2r3_gilles_fields = np.array([15.2, 17.5, 19, 19, 23 ])
...
for i in range(len(angles)):
    ax4.scatter(angles[i], r1r2_ac_fields[i], facecolor='none', edgecolor=colours[i], s=200, marker='D')

for i in range(len(r1r2_gilles_fields)):
    ax4.scatter(angles[i], r1r2_gilles_fields[i], c=colours[i], s=200, marker='+')
    ax4.scatter(angles[i], r2r3_gilles_fields[i], facecolor='none', edgecolor=colours[i], s=200, marker='h')
