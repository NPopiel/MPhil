...
f = lambda x, a, b, c, d : a * x ** 2 * b * np.exp(c * x) + d

for i, s_num in enumerate(filenumbers):
    B, VT15 = load_xy(resistance_path + file_root + str(s_num) + file_end, str(s_num), x_root=field_root,
    ...
    b_over = np.linspace(0, 41.5, 1000)

    ax1.plot(B, -1 * VT15, linewidth=2, c='k', label='Today -14')
    ax2.plot(B ** 2, -1 * VT15, linewidth=2, c='k', label='Today -14')

    B, VT15 = load_xy(resistance_path + file_root + str(119) + file_end, str(119), x_root=field_root,
                      y_root=VT15_root, skiprows=9)
    ...
    ax1.plot(B, -1 * VT15, linewidth=2, c='darkgray', label='Today -14 quick')
    ax2.plot(B ** 2, -1 * VT15, linewidth=2, c='darkgray', label='Today -14 quick')

    x, y = load_xy('/Volumes/GoogleDrive/Shared drives/Quantum Materials - Sebastian Group/MagnetTime/2019_10_TLH/TLH-hybrid-2019Oct.111.txt',
                          str(111), x_root=field_root,
                          y_root='Lockin_VT15_in_', skiprows=9)

    ax1.plot(x,y - y[0], linewidth=2, c='r', alpha=0.6, label='OG Hybrid 14')
    ax2.plot(x ** 2, y - y[0], linewidth=2, c='r', alpha=0.6, label='OG Hybrid 14')

    x, y = load_xy(
        '/Volumes/GoogleDrive/Shared drives/Quantum Materials - Sebastian Group/MagnetTime/2019_10_TLH/TLH-hybrid-2019Oct.114.txt',
        str(114), x_root=field_root,
        y_root='Lockin_VT15_in_', skiprows=9)

    ax1.plot(x, y - y[0], linewidth=2, c='darkgreen', alpha=0.6, label='OG Hybrid 16')
    ax2.plot(x ** 2, y - y[0], linewidth=2, c='darkgreen', alpha=0.6, label='OG Hybrid 16')

    x, y = load_xy(
        '/Volumes/GoogleDrive/Shared drives/Quantum Materials - Sebastian Group/MagnetTime/2019_10_TLH/TLH-hybrid-2019Oct.104.txt',
        str(104), x_root=field_root,
        y_root='Lockin_VT15_in_', skiprows=9)

    ax1.plot(x, y - y[0], linewidth=2, c='purple', alpha=0.6, label='OG Hybrid 10')
    ax2.plot(x ** 2, y - y[0], linewidth=2, c='purple', alpha=0.6, label='OG Hybrid 10')




publication_plot(ax1, r'$B$ (T)', r'Torque (V)')
publication_plot(ax2, r'$B^2$ (T$^2$)', r'Torque (V)')


ax1.set_xbound(0,46)
handles, labels = ax1.get_legend_handles_labels()

legend = ax1.legend(handles, labels, framealpha=0, ncol=1, loc='best',
