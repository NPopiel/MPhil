...
for i, filename in enumerate(filenames):
        ...
        freq = frequencys[i]
        ax4.scatter(angles[i], freq, marker='o', c=colours[i], s=275)
        ...
        ax3.plot(mega_fft[:,0] / 1e3, (mega_fft[:,1] / np.max(mega_fft[:,1])) + i * 0.9, linewidth=1.8, c=colours[i])
        ...
        for j, f in enumerate(frequencys_open_circles[i]):
            ...
            if f is not None:
                ...
                ax4.scatter(angles[i], f, marker='o', edgecolor=colours[i], facecolor='none', s=200)

        for j, f in enumerate(questionable_frequencys[i]):

            if f is not None:

                ax4.scatter(angles[i], f, marker='2', c=colours[i], s=200)
...
ax1.annotate(r'a', xy=(-0.05, 1.15), xycoords='axes fraction',
             ha="left", va="center", fontname='arial', fontweight='bold', fontsize=28)

ax2.annotate(r'b', xy=(-0.05, 1.05), xycoords='axes fraction',
             ha="left", va="center", fontname='arial', fontweight='bold', fontsize=28)

ax3.annotate(r'c', xy=(-0.05, 1.05), xycoords='axes fraction',
             ha="left", va="center", fontname='arial', fontweight='bold', fontsize=28)

ax4.annotate(r'd', xy=(-0.05, 1.05), xycoords='axes fraction',
             ha="left", va="center", fontname='arial', fontweight='bold', fontsize=28)


# fig.add_axes([0,-55,1,1]).axis("off")
plt.tight_layout(pad=.5)
plt.savefig('/Volumes/GoogleDrive/Shared drives/AGE_JRF/Magnet times/TLH_SCM4_Jul22/Figs/new_figs/c2b-w-mega-v2.pdf'
