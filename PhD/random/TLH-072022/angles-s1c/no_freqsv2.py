...
filenames = [#path+'0.035K_192deg_sweep306_up.csv',
            ...
            '/Volumes/GoogleDrive/Shared drives/AGE_JRF/Magnet times/TLH_SCM4_Jul22/S1c/0.035K_174deg_sweep257_down.csv',
...
for i, filename in enumerate(filenames):
        ...
        fft_xs.append(fft_x)
        fft_ys.append(fft_y)
        ax2.plot(new_field, subtracted_tau * 1e6 + i * .59, linewidth=1.4, c=colours[i],
                 label=str(angles[i]) + r'$\degree$')

        if fft_files[i] is not None:

            mega_fft = np.loadtxt(fft_files[i], skiprows=1, delimiter='\t')
            ax3.plot(mega_fft[:, 0] / 1e3, (mega_fft[:, 1] / np.max(mega_fft[:, 1])) + i * 0.9, linewidth=1.8,
                     c=colours[i])
        else:
            ax3.plot(fft_x / 1e3, (fft_y / np.max(fft_y)) + i * 0.9, linewidth=1.8,
                     c=colours[i])
        ...
        ax3.annotate(str(angles[i])+r'$\degree$', xy=(7.84, i*0.9+0.1), xycoords='data',
                     fontname='arial',fontsize=22,color=colours[i])
        ax2.annotate(str(angles[i]) + r'$\degree$', xy=(27.95, i * .59), xycoords='data',
                     fontname='arial', fontsize=22, color=colours[i])
...
ax2.set_ybound(-0.5, 5.45)

# ax2.set_xbound(25,28)
# ax2.set_xticks([25, 26,27,28])

ax3.set_ybound(-0.02,8.25)
ax3.set_xbound(0,10)
...
ax1.annotate(r'$T = 30$ mK ', xy=(0.02, 0.1), xycoords='axes fraction',
...
plt.savefig('/Volumes/GoogleDrive/Shared drives/AGE_JRF/Magnet times/TLH_SCM4_Jul22/Figs/new_figs/c2b-w-mega-v3-w9.pdf'
