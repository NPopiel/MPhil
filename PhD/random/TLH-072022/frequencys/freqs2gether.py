...
questionable_frequencys = [(3.244, 2.1, None, None), # 6
                           (4.47, 6.16, 2.426, None), # 18
                           (2.181, 5.315, 6.95, None), # 30
                           (7.203, None, None, None), # 36
                           (6.873, None, None, None), # 54
                           (None, None, None, None), # 66
                           (None, None, None, None), # 78
                           (1.635, 4.852, 2.917, 5.37)]# 84
...
for i, filename in enumerate(filenames):
        ...
        freq = frequencys[i]
        ax1.scatter(angles[i], freq, marker='o', c=colours[i], s=275)
        ...
        for j, f in enumerate(frequencys_open_circles[i]):
            ...
            if f is not None:
                ...
                ax1.scatter(angles[i], f, marker='o', edgecolor=colours[i], facecolor='none', s=200)

        for j, f in enumerate(questionable_frequencys[i]):

            if f is not None:

                ax1.scatter(angles[i], f, marker='2', c=colours[i], s=200)
...
for i, filename in enumerate(filenames):
        ...
        for j, f in enumerate(freqs):
            ...
            if f is not None:
                ...
                ax2.scatter(angles[i], f, marker=markers[j], c=colours[i], s=200)

ax2.scatter(angles[-1], 18.12, marker='x', c=colours[-1], s=200)
...
ax1.annotate(r'$90 \degree\Rightarrow\mathbf{H}\parallel[010]$ ', xy=(0.435, 0.05), xycoords='axes fraction',
             ha="right", va="center", fontname='arial', fontsize=20)

ax1.annotate(r'$0 \degree\Rightarrow\mathbf{H}\parallel [001]$ ', xy=(0.435, 0.1), xycoords='axes fraction',
             ha="right", va="center", fontname='arial', fontsize=20)

ax2.annotate(r'$90 \degree\Rightarrow\mathbf{H}\parallel[100]$ ', xy=(0.435, 0.05), xycoords='axes fraction',
             ha="right", va="center", fontname='arial', fontsize=20)

ax2.annotate(r'$0 \degree\Rightarrow\mathbf{H}\parallel [001]$ ', xy=(0.435, 0.1), xycoords='axes fraction',
             ha="right", va="center", fontname='arial', fontsize=20)
...
plt.savefig('/Volumes/GoogleDrive/Shared drives/AGE_JRF/Magnet times/TLH_SCM4_Jul22/Figs/new_figs/freqs-v1-20kT.pdf'
