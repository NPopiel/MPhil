...
ute2_crystal_path = '/Volumes/GoogleDrive/My Drive/UTe2-crystal.png'  # '/Users/npopiel/Desktop/UTe2-crystal.png'
...
temperatures = [27, 44, 61, 83, 100, 140]  # These are definitely wrong and mass is much heavier as a consequence
...
oversampled_temps = np.linspace(0.0001, 0.2, 10000)
...
ax3.plot(oversampled_temps * 1e3, lk1 / norm_const, c='darkslategray', label=r'$F_1$', linestyle='dashed',zorder=-1)
ax3.plot(oversampled_temps * 1e3, lk2 / norm_const, c='darkslategray', label=r'$F_2$',zorder=-1)
...
ax3.annotate(r'$m^*_1 = 78(2)$ $m_e$', xy=(0.26, 0.61), xycoords='axes fraction',
             ha="left", va="center", fontname='arial', fontsize=18)

ax3.annotate(r'$m^*_2 = 57(3)$ $m_e$', xy=(30 / 400, 0.08), xycoords='axes fraction',
             ha="left", va="center", fontname='arial', fontsize=18)

publication_plot(ax3, 'Temperature (mK)', '')
ax3.set_yticklabels([])
#
# ax_raw = ax3.inset_axes([75, 0.42, 100, .4], transform=ax3.transData)
#
# for i, filename in enumerate(filenames):
#     dat = load_matrix(path + filename)
#
...
#
# # Draw image
# axin = ax3.inset_axes([100, 0.21, 90, .75], transform=ax3.transData)  # create new inset axes in data coordinates
# axin.imshow(arr_image)
# axin.axis('off')
import matplotlib as mpl

# read image file
with mpl.cbook.get_sample_data('/Volumes/GoogleDrive/Shared drives/AGE_JRF/Magnet times/TLH_SCM4_Jul22/Figs/9deg-removebg-preview.png') as file:
    arr_image = plt.imread(file, format='png')

# Draw image
axin = ax2.inset_axes([0.65, 0.2, 0.25, 0.25])  # create new inset axes in data coordinates
axin.imshow(arr_image)
axin.axis('off')

axin.annotate(r'$\phi = 9 \degree$', xy=(0.25, 0.47), xycoords='axes fraction',
