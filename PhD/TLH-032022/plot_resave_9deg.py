...
files = [
'/Users/npopiel/Desktop/Hybrid/9degup.csv',
'/Users/npopiel/Desktop/Hybrid/9degdown.csv']
...
for i, f1 in enumerate(files):
    ...
    volts = 1e3*medfilt(np.genfromtxt(f1, delimiter=',')[5:, 1],31)

    inds = field>0
    field = field[inds]
    volts = volts[inds]

    new_field, new_volts = interp_range(field, volts, 10.01, 41.5)


    ax.plot(new_field, new_volts, lw=2, c='midnightblue', label=str(labels[i]), linestyle=linestyles[i])

    deriv = savgol_filter(new_volts, tesla_window(new_field,3), 3, 1)
    np.savetxt('/Users/npopiel/Desktop/Hybrid/3T-filter-9deg-' + labels[i] + '.csv', np.array([new_field,new_volts,deriv]).T,delimiter=',')

    ax2.plot(new_field, 1e6*deriv, c='midnightblue',lw=2, linestyle=linestyles[i])
...
legend = ax.legend(handles, labels, framealpha=0, ncol=1, loc='best',
