...
def extract_moving_indices(field, n=.1):
    ...
    while first_field <= max_field-3:
        first_field = np.round(last_field,1)
...
def fit_exp_as_function(inds_list, field, volts, plot=False):
    ...
    for ind_set in inds_list:
        ...
        popt, pcov = curve_fit(exp, B, v,maxfev=150000)
        ...
        if plot:
            ...
            ax.plot(B, exp(B,a,b, c), lw=2, c=select_discrete_cmap('venasaur')[6], linestyle='dashed')
            print(b)
            plt.show()
...
for i, f1 in enumerate(files):
    ...
    inds_list = extract_moving_indices(field,n=1)

    param_array = fit_exp_as_function(inds_list, field, volts, plot=True)
    ...
    lambdas.append(savgol_filter(param_array[:,2], 5, 3))
