...
def get_test_stat_b(full_field, full_volts, param_array):
    ...
    for j in range(len(param_array[:, 1])):
        residuals = full_volts - (param_array[j,1] * full_field ** 2 + param_array[j,2] * full_field + param_array[j,3])
...
for i, file in enumerate(files):
    ...
    for j in np.arange(len(param_array[:, 1])-1)[2:]:
    ...
    a2.scatter(angles_number[i], param_array[np.argmin(test_stat[3:]),0]+3, c=plt.cm.plasma(i / len(angles)), s=200)
