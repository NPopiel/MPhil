...
fig2, a = MakePlot(gs=True, ncols=2, nrows=2, figsize=(8,8)).create()
...
for i, file in enumerate(files):
    ...
    std_2nd_deriv_og = np.std(second_deriv[N:2*N])

    mean_3rd_deriv_og = np.mean(third_deriv[N:2 * N])
    std_3rd_deriv_og = np.std(third_deriv[N:2 * N])

    dev_locs = np.abs(second_deriv) > 4*np.abs(mean_2nd_deriv_og) #+ 25 * std_2nd_deriv_og
    # dev_locs = np.abs(third_deriv) > 4*np.abs(mean_3rd_deriv_og) #+ 25 * std_2nd_deriv_og
    ...
    axs[1].plot(field, second_deriv, c='purple', alpha=0.5,linewidth=2)
    # axs[1].plot(field, third_deriv, c='purple', alpha=0.5,linewidth=2)
    # axs[1].axhline(mean_3rd_deriv_og, linewidth=2, linestyle='dashed', c='darkorange', alpha=0.5)
    ...
    publication_plot(axs[0], '', 'Raw Torque (arb,)',title=angles[i])
