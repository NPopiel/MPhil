...
def fit_quad_as_function(inds_list, field, volts, plot=False):
    ...
    for ind_set in inds_list:
        ...
        if plot:
            ...
            ax.plot(B,v,lw=2, c=select_discrete_cmap('venasaur')[0])
            ax.plot(B, np.poly1d(f)(B), lw=2, c=select_discrete_cmap('venasaur')[6], linestyle='dashed')
            plt.show()

        lst.append([np.max(B), a,b, c, np.sum(ind_set)])

    return np.array(lst)

def get_test_stat_b(full_field, full_volts, inds, param_array):

    test_stat = []

    for j in range(len(param_array[:, 1])):
        residuals = full_volts[inds[j]] - (param_array[j,1] * full_field[inds[j]] ** 2 + param_array[j,2] * full_field[inds[j]] + param_array[j,3])

        # residuals /= np.mean(full_volts[inds[j]])

        sum_squares = np.sum(np.square(residuals))

        dof = param_array[j,4] - 3
...
for i, file in enumerate(files):
    ...
    test_stat = get_test_stat_b(field, volts, fitting_poly_inds, param_array)
