...
for i, file in enumerate(files):
    ...
    tau = savgol_filter(median_filter(1e3*np.genfromtxt(file, delimiter=',')[:,1],med_num),savgol_num,5)
    tau_copy = tau
    tau = tau[B_inds]
    ...
    deriv = savgol_filter(volts, savgol_num,3,deriv=1)
    second_deriv = savgol_filter(deriv, savgol_num, 3, deriv=1)
    # second_deriv = np.diff(deriv)

    mean_2nd_deriv_og = np.mean(second_deriv[:N])
    std_2nd_deriv_og = np.std(second_deriv[:N])

    dev_locs = np.abs(second_deriv) > 5*np.abs(mean_2nd_deriv_og) #+ 25 * std_2nd_deriv_og
    ...
    publication_plot(axs[0], '', 'Raw Torque (arb,)')
