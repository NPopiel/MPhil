...
for i, f1 in enumerate(files):
    ...
    volts = savgol_filter(volts,121,2,deriv=2)
    #field = field[:-2]
    fields.append(field)
    vs.append(volts)
...
#array = np.abs(np.array(vs))

# array[array>3] = 3

interpd = scipy.interpolate.griddata((np.concatenate(big_list_angles), np.concatenate(fields)), np.concatenate(vs), (grid_x, grid_y), method='linear')#, rescale=True)
