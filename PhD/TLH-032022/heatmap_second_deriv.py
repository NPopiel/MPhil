...
for i, f1 in enumerate(files):
    ...
    inds_list = extract_moving_indices(field,n=0.5, window_size=1.5)

    param_array = fit_poly_as_function(inds_list, field, volts)

    fits_by_files[angles[i]] = param_array
    #
    # fig, ax = MakePlot().create()
    #
    # ax.plot(param_array[:,0], param_array[:,1])
    #
    # plt.show()
...
interpd = scipy.signal.medfilt2d(interpd,25)


# interpd[interpd<-3e-6]=0
# interpd[interpd>3e-6]=0
