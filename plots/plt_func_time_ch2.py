...
for idx, folder in enumerate(folder_exts):
    ...
    for temp in range(t1,t2):
        ...
        sns.scatterplot(y='resistance_ch2', x='time',hue='current_sweep_ch2', data=df, ax=ax1,legend=False)
        #ax1.set_title('Resistance by Time')
        ax1.set_ylabel(r'$R  $', usetex=True, rotation=0)
        ax1.set_xlabel('')

        sns.scatterplot(y='ac_current_ch2', x='time', hue='current_sweep_ch2', data=df, ax=ax2,legend=True)
        #ax2.set_title('Resistance by Time')
        ax2.set_ylabel(r'$I  $', usetex=True, rotation=0)
        ax2.set_xlabel('')

        sns.scatterplot(y='voltage_amp_ch2', x='time',hue='current_sweep_ch2', data=df, ax=ax3,legend=False)
        #ax3.set_title('Voltage by Time')
        ax3.set_ylabel(r'$V  $', usetex=True, rotation=0)
        ax3.set_xlabel('')

        sns.scatterplot(y='phase_angle_ch2', x='time', hue='current_sweep_ch2', data=df, ax=ax4,legend=False)
        #ax4.set_title('Resistance by Time')
        ax4.set_ylabel(r'$\phi  $', usetex=True, rotation=0)
        ax4.set_xlabel('')

        sns.scatterplot(y='b_field', x='time', hue='current_sweep_ch2', data=df, ax=ax5,legend=False)
        #ax5.set_title('Magnetic Field by Time')
        ax5.set_ylabel(r'$B  $', usetex=True, rotation=0)
        ax5.set_xlabel('')

        sns.scatterplot(y='sigma_resistance_ch2', x='time', hue='current_sweep_ch2', data=df, ax=ax6,legend=False)
        #ax6.set_title(r'$\sigma_R$  by Time', usetex=True)
        ax6.set_ylabel(r'$\sigma_R$', usetex=True, rotation=0)
        ax6.set_xlabel('')

        sns.scatterplot(y='in_phase_voltage_ch2', x='time', hue='current_sweep_ch2', data=df, ax=ax7,legend=False)
        #ax7.set_title('In Phase Voltage by Time')
        ax7.set_ylabel(r'$V_{\phi}  $', usetex=True, rotation=0)
        ax7.set_xlabel(r'$t (s)$', usetex=True)

        sns.scatterplot(y='quad_voltage_ch2', x='time', hue='current_sweep_ch2', data=df, ax=ax8,legend=False)
        ...
        sns.scatterplot(y='gain_ch2', x='time', hue='current_sweep_ch2', data=df, ax=ax9,legend=False)
