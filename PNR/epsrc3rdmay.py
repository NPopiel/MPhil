...
def fit_line(mag, field, abs_val_h=3):
...
def langevin(field, mu_eff, c_imp):
    return c_imp * mu_eff * (
                1 / np.tanh(np.array(mu_eff * field / 1.8 / kb)) - 1 / (np.array(mu_eff * field / 1.8 / kb)))


def subtract_background(df):
    mag = scipy.ndimage.median_filter(np.array(df['DC Moment Fixed Ctr (emu)']),
                                      size=5)  # scipy.ndimage.filters.median_filter(np.array(field_df['DC Moment Fixed Ctr (emu)']), size=3)/1000
...
def filter_tempsweeps(df, size=5):
    ...
    mag = scipy.ndimage.median_filter(mag, size=size)

    return temp, mag


main_path = '/Volumes/GoogleDrive/My Drive/PNR/EPRSC/'
...
ax1.plot(field_base_fc, mag_base_fc - slope_base_fc * field_base_fc, c='purple',linewidth=2.5, label='0.05 T Cool')
ax1.plot(field_base_zfc, mag_base_zfc - slope_base_zfc * field_base_zfc, c='orange',label='0 T Cool')
...
ax2.plot(field_room, mag_room - slope_room * field_room, color='k', linewidth=2.5)
...
plt.suptitle('PNR Magnetisation EPSRC', fontsize=18, fontname='Times')
