...
def fit_line2(mag,field,abs_val_h=6):
...
def fit_line3(mag,field,abs_val_h=2):

    linear_h_top = np.where(field>abs_val_h)
    linear_h_bot = np.where(field<-1*abs_val_h)

    locs = np.squeeze(np.hstack([linear_h_bot,linear_h_top]))

    upper_fit = np.polyfit(field[locs],mag[locs],deg=1)
    #lower_fit = np.polyfit(field[linear_h_bot],mag[linear_h_bot],deg=1)
...
def fit_line(mag,field,abs_val_h=3.0):
...
for ind, file in enumerate(filenames):
    ...
    #magnetisation_fc = scipy.ndimage.median_filter(scipy.ndimage.median_filter(magnetisation_fc,5),5)
    field_fc = field_fc
    ...
    lines_fc.append(slope_fc*field_fc)
...
for i, t in enumerate(T):
    ...
    axs[i].set_ylabel(r'Magnetization (emu)', fontsize=16,fontname='arial')
    ...
    axs[i].yaxis.offsetText.set_fontsize(12)
    axs[i].set_title(str(t) + ' K',fontname='arial', fontsize=24)


    plt.setp(axs[i].get_xticklabels(), fontsize=16, fontname='arial')
    plt.setp(axs[i].get_yticklabels(), fontsize=16, fontname='arial')
