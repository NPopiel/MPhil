...
for ind, file in enumerate(filenames):
    ...
    magnetisation_fc = magnetisation_fc[increase_pos]
    field_fc = field_fc[increase_pos]

    if file == 'VT66-c2-300-FS.dat':
        magnetisation_fc = magnetisation_fc[:-2]
        field_fc = field_fc[:-2]

    magnetisation_fc = savgol_filter(savgol_filter(magnetisation_fc,15,1),15,1)

    magnetisation_fc = magnetisation_fc[6:]
    field_fc = field_fc[6:]
