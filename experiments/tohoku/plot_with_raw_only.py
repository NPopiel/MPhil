...
def process_data(main_path, filename_open, field_window=2.2, ref_C0=0):
    ...
    raw_cap = raw_cap + delta

    raw_cap_good_locs = raw_cap> 0.8

    raw_cap = raw_cap[raw_cap_good_locs]
    field = field[raw_cap_good_locs]
    R = R[raw_cap_good_locs]
...
for ind, filename_open in enumerate(filenames):
    ...
    ax = axs[0]
    ax.scatter(field, raw_cap, c=plt.cm.jet(ind / len(filenames)), label=angles[ind])
...
for ind, filename_open in enumerate(filenames_2ndsweep):
    ...
    ax = axs[1]
    ax.scatter(field, raw_cap, c=plt.cm.plasma(ind / len(filenames_2ndsweep)), label=angles_2[ind])
    # ax.plot(field, raw_cap[15:],lw=2.5, c=plt.cm.jet(ind/len(filenames)))
    xlabel = 'Magnetic Field (T)'
    ylabel = 'Capacitance (pF)'
    publication_plot(ax, xlabel, ylabel, tick_fontsize=16)
